    ...
    tabItems(
      ...
      tabItem(tabName = "howto",
              ...
              fluidRow(class="box_align_layout",
                ...
                box(width=6, title="Filtering",
                    whatViz("This dashboard utilizes selection and filtering systems. The main filters are in the sidebar when expanded."),
                    ...
                    p("See description on selection and filters bellow.")
                )
              ),
              fluidRow(class="box_align_layout",
                       box(width=12, title="Filtering: Selection",
                           whatViz("Selection refers to the group of filters that select the articles to inspect. These are: UUID, Article name and a hidden n-recent filter."),
                           whyViz("Selecting specific articles will let you inspect any data relevant to the topic you are researching."),
                           howViz("Clear selection: use 'CLEAR SELECTION'-button to clear selection."),
                           howViz("Clear all: use 'CLEAR ALL'-button to clear selection and filters."),
                           howViz("Sync article names and UUID: Click 'Syncronize artciles and id'-button."),
                           howViz("Remove any selection in selections: click on a value you want to remove and press Backspace-key or Delete-key to remove it. Hold down Ctrl to select multiple. Alternatively, select input-field (e.g. by clicking or using tab button), and navigate the cursor with the arrow keys, and use Backspace-key or Delete-key to remove Articles. Or use clear buttons"),
                           span(helpText("It's recomended to use filters first, before manually adjusting selection.")),
                           br(),
                           whatViz("UUID"),
                           whyViz('Filtering by uuid is what the system identifies as unique identifier, and is optimal for coding purposes.'),
                           howViz("Write the bazo article UUID for a given article and select with either mouse or highligt with keyboard and press enter."),
                           br(),
                           whatViz("Article name"),
                           whyViz("Filtering by article names is what is most logical for most users, as they can easily be found by browsing tv2fyn.dk."),
                           howViz("Type the name of an Article. Press enter to add or select suggestion by clicking on it."),
                       ...
                       box(width=12, title="Filtering: Filters",
                           ...
                           howViz("Drag the slider to the amout you want to inspect. Selecting a number higher than the amount of available articles will display all available."),
                           br(),
                           whatViz("Topic"),
                           whyViz("To inspect topics from the topic tab. It uses STM, so the topics are can be latent or random."),
